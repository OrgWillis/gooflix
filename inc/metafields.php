    ...
    class Doofields {
        ...
        public function fields_html($item) {
            if(is_array($item)){
                ...
                // Compose view
                $response = apply_filters('mega_metafields', $output, $type);
                // Echo View
                echo $response;
            }
        }
        ...
        public function textarea($args) {
            ...
            $btnt   = $upload ? __('Upload','mega') : false;
            // View
            $output  = "<tr id='{$id}_box'><td class='label'><label for='{$id}'>{$label}</label>";
    		$output .= "<p class='description'>{$desc}</p></td>";
        ...
        public function generator($args) {
            ...
            $text_buttom  = ($editoraction == 'edit') ? __('Update info','mega') : __('Generate','mega');
            $acti_buttom  = ($editoraction == 'edit') ? 'dbmovies-updaterpost' : 'dbmovies-generartor';
            $text_duplic  = __('Check duplicate content','mega');
            // Values
            $value1 = $id  ? $this->meta($id)  : false;
            $value2 = $id2 ? $this->meta($id2) : false;
        ...
        public function upload($args){
            ...
            $value   = $id ? $this->meta($id) : false;
            $btntext = __('Upload now','mega');
            $btnuplo = __('Upload','mega');
            // View
